...
void SystemInit (void) {
  ...
  SCB_NS->CPACR |= ((3UL << 10*2) | (3UL << 11*2));    /* set CP10, CP11 Full Access in Non-secure mode */
	...
	// we cannot use memcpy here as the std lib might not be ready at this point
//    memcpy((void *)flexspi_nor_config, (void *)FLASH_CONFIG_ADDRESS, sizeof(flexspi_nor_config));
	for (uint32_t i=0; i<sizeof(flexspi_nor_config)/sizeof(uint64_t); i++)
		((uint64_t *)flexspi_nor_config)[i] = ((uint64_t *)FLASH_CONFIG_ADDRESS)[i];
